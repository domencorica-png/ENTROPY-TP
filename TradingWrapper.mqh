...
public:
             ...
             bool enableAdaptiveTP = false)  // Parametro aggiuntivo per TP adattativo
    {
        ...
        if(m_enableAdaptiveTP)
        {
            ...
            // Valori di default ottimizzati per XAUUSD 10min
            tpConfig.long_a = 1.2;
            tpConfig.long_b = 0.8;
            tpConfig.short_a = 1.3;
            tpConfig.short_b = 0.7;
            tpConfig.fixed_cap_percent = 0.65;   // 0.65%
            tpConfig.min_threshold_percent = 0.2; // 0.2%
            tpConfig.fallback_percent = 0.4;     // 0.4% (valore attuale)
...
private:
    ...
    // Aggiorna il filtro entropico - OTTIMIZZATO per aggiornare solo su nuove barre
    void UpdateEntropyFilter()
    {
        datetime current_time = TimeCurrent();
        datetime current_bar_time = iTime(m_symbol, m_timeframe, 0);

        // OTTIMIZZAZIONE: Aggiorna solo su nuova barra o dopo intervallo, anche in backtesting
        // Questo riduce drasticamente il numero di aggiornamenti durante i backtest
        #ifdef __MQL5__
        if(MQLInfoInteger(MQL_TESTER))
        {
            // In backtest, aggiorna solo se è una nuova barra
            if(m_symbolConfig.last_bar_time != current_bar_time)
            {
                m_entropyFilter.Update();
                m_symbolConfig.last_bar_time = current_bar_time;
    ...
    bool ShouldBlockTrade()
    {
        ...
        // NON aggiornare qui - è già stato fatto in UpdateEntropyFilter()
        // Rimuovendo questa chiamata si evitano 2 aggiornamenti ridondanti per tick!
