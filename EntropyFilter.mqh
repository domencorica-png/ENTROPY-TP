...
public:
    ...
    void Update()
    {
        ...
        // Calcola l'entropia per ogni timeframe - usa logaritmo in base 2
...
private:
    ...
    bool UpdateReturns()
    {
        ...
        // Pulisci i buffer prima di riempirli
        ArrayInitialize(m_returns_breve, 0.0);
        ArrayInitialize(m_returns_medio, 0.0);
        ArrayInitialize(m_returns_lungo, 0.0);
    ...
    double CalculateEntropy(const double &returns[], int period)
    {
        ...
        for(int i = 0; i <= m_bins; i++)
        {
            ...
            if(probability > 0)
            {
                ...
                entropy -= probability * (MathLog(probability) / MathLog(2));
            }
        }
        
        // Normalizza l'entropia tra 0 e 1
        double max_entropy = MathLog((double)(m_bins + 1)) / MathLog(2);
        if(max_entropy > 0)
